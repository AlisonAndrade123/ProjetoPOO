...
import javafx.scene.image.ImageView; // <<< IMPORT CORRIGIDO
import javafx.scene.layout.HBox;
import javafx.scene.layout.Pane; // <<< IMPORT CORRIGIDO
...
public class ProdutosController {
    ...
    public void initialize() {
        ...
        // Garante que o alerta personalizado esteja escondido ao iniciar
        hideCustomAlert();
    }
    ...
    private void handleComprarProduto(Produto produto) {
        ...
        // Agora chamamos nosso alerta personalizado
        showCustomAlert("Produto Adicionado", "Você adicionou '" + produto.getNome() + "' ao carrinho!");
    }

    @FXML private void handleCartButtonAction(ActionEvent event) { NavigationManager.getInstance().navigateToCart(); }
    @FXML private void handleProfileButtonAction(ActionEvent event) { showCustomAlert("Funcionalidade", "Funcionalidade de Perfil ainda não implementada."); }

    @FXML
    private void handleCustomAlertOK(ActionEvent event) {
        hideCustomAlert();
    }

    // --- MÉTODOS DO ALERTA PERSONALIZADO ---
    ...
    // --- O resto da classe (criação de botões, cards, filtros) ---
    ...
    private void loadAllProducts() {
        if (produtoDAO != null) {
            try { List<Produto> produtos = produtoDAO.findAll(); displayProducts(produtos); }
            catch (SQLException e) { showCustomAlert("Erro de Banco de Dados", "Erro ao carregar produtos."); e.printStackTrace(); }
        }
    }

    private void filterProducts(String searchText) {
        if (produtoDAO != null) {
            try { List<Produto> produtos = produtoDAO.search(searchText); displayProducts(produtos); }
            catch (SQLException e) { showCustomAlert("Erro de Banco de Dados", "Erro ao pesquisar produtos."); e.printStackTrace(); }
        }
    }

    private void filterProductsByCategory(String category) {
        if (produtoDAO != null) {
            try { List<Produto> produtos = produtoDAO.findByCategory(category); displayProducts(produtos); }
            catch (SQLException e) { showCustomAlert("Erro de Banco de Dados", "Erro ao filtrar produtos."); e.printStackTrace(); }
        }
    }
